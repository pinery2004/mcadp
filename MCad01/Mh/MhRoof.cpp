...
{
	GLn3->n = 0;
	return MgDivideAdd( Sel, SLn1, Pg2, GLn3);
}
*/

static void MgAndGLineOnSLineAdd(
...
static bool MgCheckRfmCls(							// (  O) true:	交差チェック対象屋根面
													//		 false:	交差チェック対象外屋根面
...
//    地廻り線を外から見て右側に接続する屋根構成線（延長線分）を作成する 
	...
	for ( ic1=0; ic1<m_GJim1.m_n; ic0=ic1,ic1++) {
		...
		ist = MGeo::Intr2Pln3D( Pln0, Pln1, &SLn1);				// 屋根面間の交線を求める
			ASSERT( ist == MC_INT);								// 寄棟部構成線作成エラー　<ERROR>
		if ( SLn1.v.z < 0.0)									// 交線の方向を上向きにする
			SLn1.v = - SLn1.v;
		P0 = MgPoint3DC( m_GJim1[ic0].m_p);
		P1 = P0 + MCMAXIESIZE * SLn1.v;
...
//			ASSERT(m_GRfm.m_st[ic2].pg.n != 0);
			...
			ist1 = MGeo::Intr2Pln3D( *pPln1, *pPln2, &SLn0);			// 二つの屋根面の交差直線を求める
																// 屋根面は地廻り線の順に並んでおり交差直線の方向は
																// ２次元にした時にxが大から小の方向に向かっている
			if ( ist1 != MC_INT) 
				continue;

			MGeo::SLine3Dto2D( SLn0,  MatXY, &sln1);					// 交差直線を２次元に変換し
			ist1 = MGeo::DivideSLinePolygon2D( MC_IN_BORDER | MC_ON_BORDER , sln1, pg1, &Gln1);
...
//				sel1 = MC_IN_BORDER | MC_ON_BORDER;
			...
			ist1 = MGeo::DivideSLinePolygon3D( sel1, SLn0, m_GRfm.m_st[ic2].m_Pg, &GLn2);
...
//							Gln2.Print( Mstr( "051114 屋根面2の形状内に含まれる線分群2 Gln2"));
			...
			MgAndGLineOnSLineAdd( Gln1, Gln2, &Gln3);			// (同一直線上の)線分群1と線分群2の重なり部分を取り出す
...
//MBLOGON;
	for ( ic1=0; ic1<m_GRfm.m_n; ic1++) {
		...
		//if ( MGeo::CheckZero( pPln1->v.z))								// XY平面に垂直の屋根面はスキップ
		//	continue;
...
static bool MgCheckRfmCls(							// (  O) true:	交差チェック時に屋根面2の境界を含める
				...
				)
{
	...
	MGeo::CheckLineOnPln3DWS( LnJim1, Pln2, &ist1);							// 屋根面1の地廻り線位置が、屋根面2の上下どちらかを判定
	MGeo::CheckLineOnPln3DWS( LnJim2, Pln1, &ist2);							// 屋根面2の地廻り線位置が、屋根面1の上下どちらかを判定
