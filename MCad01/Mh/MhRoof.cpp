...
//    地廻り線に対応する屋根面を作成する 
	...
	for ( ic1=0; ic1<m_GJim1.m_n; ic0=ic1,ic1++) {
		...
		if ( m_GJim1[ic1].m_icd == MHRL_NOKI) {
			...
			RfmI.m_Pln = MgPlane3DC( m_GJim1[ic0].m_p, m_GJim1[ic1].m_p, m_GJim1[ic1].m_rKb / 10.f);	// 始点・終点･勾配
		} else if ( m_GJim1[ic1].m_icd == MHRL_KERABA) {
			RfmI.m_icd = MHRL_YAGIRI;
			Pth = MgPoint3DC( m_GJim1[ic0].m_p);
			Pth.z += 1000.;
			RfmI.m_Pln = MgPlane3DC( MgPoint3DC( m_GJim1[ic0].m_p), MgPoint3DC( m_GJim1[ic1].m_p), Pth);			// ３点
		}
		if ( RfmI.m_icd) {
			m_GRfm += RfmI;										// 屋根面追加
			int ii = sizeof( MhRfm);
...
//#endif
		...
		MGeo::Mat3PlntoXY( *pPln1, m_GRfm.m_st[ic1].m_Pg.m_P[0], &MatXY, &MatPln);	// ３Ｄ→２Ｄ用変換マトリックスと逆変換マトリックスを求める
...
//				sel1 = MC_IN_BORDER | MC_ON_BORDER;
			...
			ist1 = MGeo::DivideULnPg3( sel1, ULn0, m_GRfm.m_st[ic2].m_Pg, &GLn2);
...
void MhRoofInfo::ConnectRfm()
{
	...
	for ( ic1=0; ic1<nRfm; ic1++) {
		...
		for ( ic2=ic1+1; ic2<nRfm; ic2++) {
			...
			ist1 = MGeo::Merge2PgtoGPg3( MC_OR, m_GRfm.m_st[ic1].m_Pg, m_GRfm.m_st[ic2].m_Pg, &GPgw);
