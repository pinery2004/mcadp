...
public :
	...
	MgPoint2D		m_PtOriginG;				// グリッド原点座標
	...
	MgMinMaxR2D		m_rMinMaxG;					// グリッド領域の最小最大
	...
	bool	GetDrawFlgG()										// グリッド表示フラグを取得
						{ return m_bFlgDrawG;}
	void	SetOriginPtG( MgPoint2D PtOriginG)					// グリッド原点座標の設定
						{ m_PtOriginG = PtOriginG;}
	MgPoint2D GetOriginPtG()										// グリッド原点座標を取得
						{ return m_PtOriginG;}
	...
	void	SetMinMaxG( MgMinMaxR2D& rMinMaxG)					// グリッド領域の最小最大の設定
						{ m_rMinMaxG = rMinMaxG;} 
	MgMinMaxR2D GetMinMaxG()										// グリッド領域の最小最大を取得
						{ return m_rMinMaxG;}
	MgMinMaxR2D GetMinMaxGA(
						msCod*		pCod
				);
