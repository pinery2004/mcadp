================================================================================
MICROSOFT FOUNDATION CLASS ライブラリ: MCad プロジェクトの概要
===============================================================================

アプリケーション ウィザードが作成した MCad には Microsoft 
Foundation Class の基本的な使い方が示されています。アプリケーション作成のひな型
としてお使いください。

このファイルには MCad アプリケーションを構成している各ファイ
ルの概要説明が含まれます。
...
MCad.h
   ...
   CMCadApp アプリケーション クラスを宣言します。

MCad.cpp
   これは、アプリケーション クラス CMCadApp を含むメイン アプリケー
   ション ソース ファイルです。
...
res\MCad.ico
   ...
   アイコンはメイン リソース ファイル MCad.rc に含まれていま
   す。
...
ChildFrm1.h, ChildFrm1.cpp
   これらのファイルは、MDI アプリケーションの子ウィンドウをサポートする 
   CChildFrame1 クラスの定義と実装を行います。
...
MCadDoc.h, MCadDoc.cpp - ドキュメント
   これらのファイルは CMCadDoc クラスを含みます。特別なドキュメント 
   データを追加したり、ファイルを保存したり、
   (via CMCadDoc::Serialize) を読み込んだりするには、これらのファイ
   ルを編集してください。
    ドキュメントには次の文字列が含まれます:
        ファイルの拡張子:             mcd
        ファイルの種類のID:           MCad.Document
        メイン フレーム キャプション: MCad
        ドキュメントの種類の名前:     MCad
        フィルタ名:                   MCad Files (*.mcd)
        ファイルの新しい短い名前:     MCad
        ファイルの種類の長い名前:     MCad.Document

MCadView1.h, MCadView1.cpp - ドキュメントのビュー
   これらのファイルは CMCadView1 クラスを含みます。
   CMCadView1 オブジェクトは CMCadDoc オブジェクトを表示す
   るのに使用されます。

res\MCadDoc.ico
   このファイルは、CMCadDoc クラスの MDI 子ウィンドウ用アイコンとし
...
StdAfx.h, StdAfx.cpp
   ...
   MCad.pch や既にコンパイルされた型のファイル StdAfx.obj を
   ビルドするために使用されます。
